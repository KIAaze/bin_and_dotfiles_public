...
def vigenere(decrypted_message, key, encrypt=True, start_chr=' ', end_chr='~', lower_case=False):
  ...
  for idx, letter in enumerate(decrypted_message):
    ...
    if start_ord <= ord(letter) <= end_ord:
      ...
      if encrypt:
        new_letter_ord = (letter_delta + key_delta) % period + start_ord
      else:
        new_letter_ord = (letter_delta - key_delta) % period + start_ord
...
def main():
  ...
  args.outfile.write(vigenere(args.infile.read(), args.key, args.encrypt, args.start_chr, args.end_chr, args.lower_case))
