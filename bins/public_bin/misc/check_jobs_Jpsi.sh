...
if [ $3 -eq 2 ]
then
  echo 'checking Jpsi_MUCH jobs';
  for (( i = $1 ; i <= $2 ; i++ ))
      do
            jobname=Jpsi_MUCH_01_$i
            if [ $(grep -c $jobname joblist.txt) -eq 0 ];
            then
            echo "$jobname does not exist.";
            fi
            
            jobname=Jpsi_MUCH_10_$i
            if [ $(grep -c $jobname joblist.txt) -eq 0 ];
            then
            echo "$jobname does not exist.";
            fi
            
            jobname=Jpsi_MUCH_11_$i
            if [ $(grep -c $jobname joblist.txt) -eq 0 ];
            then
            echo "$jobname does not exist.";
            fi
