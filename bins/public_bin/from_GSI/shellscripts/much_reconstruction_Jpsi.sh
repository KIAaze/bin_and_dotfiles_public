	...
	do
            ...
            then
              ...
              then
                      ...
                      echo "index=$i: creating Jpsi_MUCH_01_$i";
                      bsub -J Jpsi_MUCH_01_$i root -l -q .x "~/cbmroot/macro/much/much_reco_Jpsi.C($1,$i,0,1)";
              fi            
            fi
            
            jobname=Jpsi_STS_10_$i
            if [ $(grep -c $jobname joblist.txt) -eq 0 ];
            then
              echo "$jobname does not exist.";
                ...
                then
                        echo "$filename does not exist.";
                        echo "index=$i: creating Jpsi_MUCH_10_$i";
                        bsub -J Jpsi_MUCH_10_$i root -l -q .x "~/cbmroot/macro/much/much_reco_Jpsi.C($1,$i,1,0)";
                fi
            fi
            
            jobname=Jpsi_STS_11_$i
            if [ $(grep -c $jobname joblist.txt) -eq 0 ];
            then
              echo "$jobname does not exist.";
                ...
                then
                        echo "$filename does not exist.";
                        echo "index=$i: creating Jpsi_MUCH_11_$i";
                        bsub -J Jpsi_MUCH_11_$i root -l -q .x "~/cbmroot/macro/much/much_reco_Jpsi.C($1,$i,1,1)";
                fi            
            fi
done
