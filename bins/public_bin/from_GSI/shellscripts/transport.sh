...
for (( i = $2 ; i <= $3 ; i++ ))
	...
	then
		...
		echo "index=$i: creating SIM_01_$i";
		bsub -J SIM_01_$i root -l -q .x "~/cbmroot/macro/much/much_sim.C($1,$i,0,1)";
	fi
	...
	then
		echo "$filename1 or $filename2 do not exist.";
		echo "index=$i: creating SIM_10_$i";
		bsub -J SIM_10_$i root -l -q .x "~/cbmroot/macro/much/much_sim.C($1,$i,1,0)";
	fi
	...
	then
		echo "$filename1 or $filename2 do not exist.";
		echo "index=$i: creating SIM_11_$i";
		bsub -J SIM_11_$i root -l -q .x "~/cbmroot/macro/much/much_sim.C($1,$i,1,1)";
	fi
